...
### 初回ひも付け（どうやって line_user_id を集めるか）

スタッフがまだ `line_user_id` 未登録だと照合できない。解決策の候補：

1. **招待コード方式（推奨）**：オーナーがスタッフに使い捨ての招待コードを渡す。
   スタッフは「LINEでログイン」→ 招待コード入力 → その時の line_user_id を
   profiles に保存してひも付け完了。以降はコード不要。
2. LINE 公式アカウントを友だち追加 → Webhook で送られてくる userId を
   オーナーが管理画面で各スタッフに割り当てる。
...
### Phase 1: 下地（DB・設定）
- [ ] `profiles.line_user_id` カラム追加（migration）
- [ ] `src/lib/line.ts`（LINE API ラッパ：token交換・push送信・署名検証）

### Phase 2: LINE ログイン
- [ ] `GET /auth/line`（state発行 → LINE認可へリダイレクト）
- [ ] `GET /auth/line/callback`（code交換・検証・Supabaseセッション発行）
- [ ] ログイン画面に「LINEでログイン」ボタン
- [ ] 招待コードによる初回ひも付け画面
- [ ] 管理画面：スタッフ詳細に line_user_id 表示／解除

### Phase 3: LINE 通知
- [ ] シフト公開時に対象スタッフへ push
- [ ] 休み希望の承認/却下時に申請者へ push
- [ ] 新規休み希望時にオーナーへ push
...
1. **本番ドメイン**（例 `shift.example.com`）と Vercel へのデプロイ
