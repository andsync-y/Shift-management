...
- [ ] （本番必須）callback の id_token 署名・aud・nonce 検証を追加（jose等）
...
> ⚠️ 本番導入前の必須対応：`exchangeLineCode` は現状 id_token を署名検証なしで
> デコードしている。本番では必ず JWKS による署名検証と aud/iss/exp/nonce 検証を
> 追加すること（`src/lib/line.ts` の TODO 参照）。
