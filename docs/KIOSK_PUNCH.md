...
- カメラは常時プレビュー（右上に小窓）。名前を押した瞬間の映像を**横480px・JPEG**で1枚取得。
...
- `time_records.source = 'kiosk'`、写真は `in_photo_url` / `out_photo_url`（`punch-photos` のパス）。
- オーナーは「勤怠管理」一覧の各行で **📷出 / 📷退** リンクから写真を確認できる（署名URL・1時間有効）。
...
- 写真の**自動削除**（例: 90日より古いものをcronで purge）で容量・privacyを抑える。
- シフト予定時刻から大きく外れた打刻の警告表示。
- 出勤のみ撮影（退勤は省略）に切替える運用オプション。
