...
- **対象日・枠は固定**（`src/lib/preopen.ts` の `PREOPEN_DAYS`）。閉店22:00まで90分刻み。
  - 6/17(水)・6/18(木)：受付開始 **15:00**（15:00 / 16:30 / 18:00 / 19:30）
  - 6/19(金)：受付開始 **13:30**（13:30 / 15:00 / 16:30 / 18:00 / 19:30）
  - 受付開始は `buildRounds(start)` で生成。空き状況表の列は全日の開始時刻の和集合 `PREOPEN_ALL_STARTS`。
